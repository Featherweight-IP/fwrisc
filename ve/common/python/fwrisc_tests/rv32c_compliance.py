...
class ComplianceTestTube(FwriscTracerBfmIF):
...
#    u_tracer.add_listener(mon)
    ...
    with open(sw_image, "rb") as f:
        ...
        self_loop = symtab.get_symbol_by_name("self_loop")[0]["st_value"]
...
#                    print("mem[" + hex(addr) + "] = " + hex(word))
                    u_sram.write_nb(int((addr & 0xFFFFF)/4), word, 0xF)
                    addr += 4
                    j += 4

    tube = ComplianceTestTube(u_sram, write_str, write_num)
    u_tracer.add_listener(tube)

    print("self_loop: " + hex(self_loop))            
    addr = await mon.wait_exec({self_loop}, 100000)
